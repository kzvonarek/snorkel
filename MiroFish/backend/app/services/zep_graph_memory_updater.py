...
async def update_agent_memory(agent_id: str, role: str, content: str):
    """
    Replaces Zep's memory updater. Appends new interactions to a Redis List.
    """
    episodes_key = f"mirofish:{agent_id}:episodes"
    ...
    await redis_db.rpush(episodes_key, interaction)
    await redis_db.ltrim(episodes_key, -100, -1)


async def summarize_memory(agent_id: str):
    """
    Placeholder for memory summarization if your agents need to compress long histories.
    """
    pass
...
class ZepGraphMemoryUpdater:
    ...
    监控模拟的actions日志文件，将新的agent活动实时更新到 Redis 中。
    按平台分组，每累积BATCH_SIZE条活动后批量写入 Redis。
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        self._total_sent = 0        # 成功写入Redis的批次数
        self._total_items_sent = 0  # 成功写入Redis的活动条数
        self._failed_count = 0      # 写入失败的批次数
    ...
    def start(self):
        ...
        self._worker_thread = threading.Thread(
            ...
            args=(current_locale,),
            daemon=True,
            name=f"RedisMemoryUpdater-{self.graph_id[:8]}"
        )
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        logger.debug(f"添加活动到Redis队列: {activity.agent_name} - {activity.action_type}")
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                for activity in activities:
                    role = activity.agent_name or activity.platform or "unknown"
                    content = activity.to_episode_text()
                    if ": " in content:
                        content = content.split(": ", 1)[1]
                    asyncio.run(update_agent_memory(self.graph_id, role, content))
                
                self._total_sent += 1
                self._total_items_sent += len(activities)
                display_name = self._get_platform_display_name(platform)
                logger.info(f"成功批量写入 {len(activities)} 条{display_name}活动到 Redis {self.graph_id}")
                logger.debug(f"批量内容预览: {' | '.join(a.to_episode_text() for a in activities)[:200]}...")
                return
                
            except Exception as e:
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"批量写入Redis失败 (尝试 {attempt + 1}/{self.MAX_RETRIES}): {e}")
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(f"批量写入Redis失败，已重试{self.MAX_RETRIES}次: {e}")
