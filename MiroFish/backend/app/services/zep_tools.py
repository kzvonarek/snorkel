...
from .zep_entity_reader import read_entity_context
...
class ZepToolsService:
    ...
    def _parse_node_record(self, entity_name: str, node_json: str) -> NodeInfo:
    ...
    def _parse_edge_record(self, edge_key: str, edge_json: str) -> EdgeInfo:
    ...
    def _load_nodes(self, graph_id: str) -> List[NodeInfo]:
        nodes = self._run_async(redis_db.hgetall(self._nodes_key(graph_id)))
        return [self._parse_node_record(entity_name, node_json) for entity_name, node_json in nodes.items()]

    def _load_edges(self, graph_id: str) -> List[EdgeInfo]:
        edges = self._run_async(redis_db.hgetall(self._edges_key(graph_id)))
        return [self._parse_edge_record(edge_key, edge_json) for edge_key, edge_json in edges.items()]

    def _find_entity_name(self, graph_id: str, query: str) -> Optional[str]:
        query_lower = query.lower().strip()
        if not query_lower:
            return None

        nodes = self._load_nodes(graph_id)
        for node in nodes:
    ...
    async def _find_node_json_by_uuid(self, node_uuid: str) -> Optional[str]:
        keys = await redis_db.keys("mirofish:*:nodes")
        for key in keys:
            value = await redis_db.hget(key, node_uuid)
            if value:
                return value
        return None
    ...
    def search_graph(
        ...
        matched_entity = self._find_entity_name(graph_id, query)
        if matched_entity:
            context = self._run_async(read_entity_context(graph_id, matched_entity))
            node = self.get_node_detail(matched_entity)
            nodes = [node.to_dict()] if node else []
            edges = [edge.to_dict() for edge in self.get_node_edges(graph_id, matched_entity)]
            return SearchResult(
                facts=[context],
                edges=edges,
                nodes=nodes,
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        result = self._load_nodes(graph_id)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        result = self._load_edges(graph_id)
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            node_data = self._run_async(self._find_node_json_by_uuid(node_uuid))
            if not node_data:
                return None

            return self._parse_node_record(node_uuid, node_data)
        except Exception as e:
    ...
    def get_entity_summary(
        ...
        all_nodes = self.get_all_nodes(graph_id)
        entity_node = next((node for node in all_nodes if node.name.lower() == entity_name.lower()), None)
