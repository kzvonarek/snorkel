<template>
  <div class="report-page">
    ...
    <div class="report-toolbar">
      <div><h2>{{ topic?.name || 'PMF simulation' }} report</h2><p>{{ run.mode === 'demo' ? 'Curated demonstration report based on the completed persona simulation.' : 'Generated from observed simulation activity and agent interviews.' }}</p></div>
...
<style scoped>
.report-page { display:flex;flex-direction:column;gap:14px;height:100%; }.report-toolbar { display:flex;justify-content:space-between;align-items:center; }.report-toolbar p { color:var(--text-3);font-size:12px;margin-top:3px; }.toolbar-actions { display:flex;gap:8px; }.report-layout { display:grid;grid-template-columns:minmax(0,1fr) 340px;gap:16px;min-height:0;flex:1; }.document,.chat-panel { background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);overflow:auto; }.document { padding:32px; }.markdown { font-family:inherit;white-space:pre-wrap;line-height:1.7;font-size:13px; }.empty-state { height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;color:var(--text-3); }.empty-state strong { color:var(--ink); }.chat-panel { display:flex;flex-direction:column; }.chat-heading { padding:13px;border-bottom:1px solid var(--border);font-weight:700; }.messages { flex:1;overflow:auto;padding:12px;display:flex;flex-direction:column;gap:10px; }.message { display:flex;flex-direction:column;gap:3px;padding:9px;border-radius:8px;background:var(--bg);font-size:12px; }.message.user { background:var(--accent-light); }.chat-form { display:flex;gap:6px;padding:10px;border-top:1px solid var(--border); }.chat-form input { min-width:0;flex:1;padding:9px;border:1px solid var(--border);border-radius:7px; }.chat-form button { padding:0 12px;background:var(--accent);color:white;border-radius:7px; }.error { padding:10px;border:1px solid #c44a4a;color:#8e2828;border-radius:8px;background:#fff0f0; }
</style>
